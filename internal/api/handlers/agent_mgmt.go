...
func GetLatestAgentHandler(w http.ResponseWriter, r *http.Request) {
	...
	`, siteID).Scan(&version.ID, &version.VersionHash, &version.ScriptContent, &version.IsActive, &version.CreatedAt)
...
func DeployAgentHandler(w http.ResponseWriter, r *http.Request) {
	...
	var exists bool
	if err := database.DB.QueryRow("SELECT EXISTS(SELECT 1 FROM sites WHERE id = $1)", req.SiteID).Scan(&exists); err != nil || !exists {
	...
	_, err = tx.Exec("UPDATE agent_versions SET is_active = false WHERE site_id = $1", req.SiteID)
	if err != nil {
	...
	err = tx.QueryRow(
		"SELECT id FROM agent_versions WHERE version_hash = $1 AND site_id = $2",
		hashStr, req.SiteID,
	).Scan(&existingID)
