...
func createTenantTables(schema string) error {
	...
	CREATE TABLE IF NOT EXISTS site_configs (
		...
		benchmark_baseline JSONB DEFAULT '{}',
	...
	CREATE TABLE IF NOT EXISTS rollout_runs (
		...
		worker_cursor INT NOT NULL DEFAULT 0,
	...
	migrations := []string{
		...
		fmt.Sprintf("ALTER TABLE %s.devices ADD COLUMN IF NOT EXISTS last_rollout_status VARCHAR(32)", quotedSchema),
		fmt.Sprintf("ALTER TABLE %s.devices ADD COLUMN IF NOT EXISTS last_rollout_at TIMESTAMP WITH TIME ZONE", quotedSchema),
		fmt.Sprintf("ALTER TABLE %s.devices ADD COLUMN IF NOT EXISTS last_health_check_at TIMESTAMP WITH TIME ZONE", quotedSchema),
		...
		fmt.Sprintf("ALTER TABLE %s.site_configs ADD COLUMN IF NOT EXISTS benchmark_baseline JSONB DEFAULT '{}'", quotedSchema),
