#!/bin/bash
scp -o StrictHostKeyChecking=no devices/agent.sh root@10.0.0.3:/tmp/agent.sh
...
ssh -o StrictHostKeyChecking=no root@10.0.0.3 "chmod +x /tmp/agent.sh && /tmp/agent.sh &"
